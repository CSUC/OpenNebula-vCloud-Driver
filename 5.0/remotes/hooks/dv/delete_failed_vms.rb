...
vmid            = drv_action["/VM/ID"]
name 			= drv_action["/VM/NAME"]
vapp_name 		= "one-#{vmid}-#{name}"
host  			= drv_action["/VM/HISTORY_RECORDS/HISTORY/HOSTNAME"]
hypervisor 		= drv_action["/VM/USER_TEMPLATE/HYPERVISOR"]

if hypervisor == "vcloud"	
	begin
    	VCloudDriver::VCloudVm.delete_failed(vapp_name,host)
	rescue Exception => e
    	STDERR.puts "Cancel of VM #{vapp_name} on host #{host} failed " +
                "due to \"#{e.message}\""
   		exit -1
	end
end
